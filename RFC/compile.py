...
from RFC.utils.functional_utils import save_object, update_output_channels, switch_output_channel, log, get_project_root
...
def setup():
    ...
    save_object(result, os.path.join(rootdir, 'references.json'))
    save_object(module_list, os.path.join(rootdir, 'module_list.json'))
    ...
    if leaves_summary_log_path:
        pprint(module_leaves_summary, stream=open(os.path.join(rootdir, leaves_summary_log_path), 'w'))

    log('\nDone.', note='Compiler', pure_output=True, end='')
...
def run_compile():
    ...
    update_output_channels({
        'compile_log': self_kwargs['compile_log_path'],
    })
