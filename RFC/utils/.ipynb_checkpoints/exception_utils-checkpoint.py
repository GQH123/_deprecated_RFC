...
class NotSupportedError(Exception):
    def __init__(self, unsupported_param, all_supported_params, module_name=None):
        self.unsupported_param = unsupported_param
        self.all_supported_params = all_supported_params
        self.module_name = module_name if module_name else get_prev_module_name()
        super(NotSupportedError, self).__init__(f'Parameter {repr(self.unsupported_param)} is not supported, all supported ones are {repr(self.all_supported_params)}. Error raised from {self.module_name}.')

    def __reduce__(self):
        return (NotSupportedError, (self.unsupported_param, self.all_supported_params, self.module_name))
...
class ParamTypeError(ParamError):
    def __init__(self, param_name, param_value, expected_types, module_name=None):
        ...
        self.module_name = module_name if module_name else get_prev_module_name()
        super(ParamTypeError, self).__init__(f"Incorrect type {repr(type(self.param_value))} of <{str(self.param_name)}={repr(self.param_value)}>, expected types are {repr(self.expected_type)}. Error raised from {self.module_name}.")

    def __reduce__(self):
        return (ParamTypeError, (self.param_name, self.param_value, self.expected_type, self.module_name))
...
class ParamSettingError(ParamError):
