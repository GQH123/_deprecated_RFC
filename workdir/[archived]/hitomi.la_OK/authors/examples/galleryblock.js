...
var nozomi = [];
var tag, area, popular, language, page_number = 1;
var total_items = 0;
                ...
                if (area) {
                        if (popular) {
                                if (area !== 'popular') {
                                        ...
                                        base = '/'+area+'/popular/'+popular+base;
                                } else {
                                        //popular/today-czech
                                        base = '/popular'+base;
                                }
                        ...
                        if (xhr.status === 200 || xhr.status === 206) {
                                ...
                                if (arrayBuffer) {
                                        ...
                                        for (var i = 0; i < total; i++) {
                                                nozomi.push(view.getInt32(i*4, false /* big-endian */));  // Note Here!
                                        }
...
function fetch_nozomiurl() {
        ...
        //series/popular/today/female:filming
        //popular/today
        var term = pathname.replace(/^\//, '').replace(/-[^-]+\.html$/, '').replace(/\/popular\/[^\/]+/, '').replace(/\/\w+:/, '/');
...
$(function() {
        $('#artistname').html(tag === 'index' ? 'Recently Added' : tag_display);
        
        start_loading_timer();
        
        var orderbydropdown = document.getElementById("orderbydropdown");
        if (orderbydropdown) {
                orderbydropdown.value = popular ? popular : 'date_added';
                ...
                orderbydropdown.addEventListener("change", function() {
                        ...
                        var url = decodeURIComponent(document.location.pathname).replace(/^\/index/, '').replace(/\/popular\/[^\/]+\//, '/').replace(/\/popular\/[^-]+/, '');
                        ...
                        if (orderbydropdown.value) {
                                if (orderbydropdown.value === 'date_added') {
                                        ...
                                        var interval = orderbydropdown.value;
                                        if (!url || url === '/') {
                                                go_to('/popular/'+interval+'-all.html');
                                        } else if (/^-/.test(url)) {
                                                go_to('/popular/'+interval+url);
                                        } else {
                                                var elements = url.split('/');
                                                elements.splice(2, 0, 'popular', interval);
                                                go_to(elements.join('/'));
                                        }
                                }
                        }
