...
def _parse_exception_type(exception, item_id):
    ...
    if 'unexpected status code 503' in exception:
        return 'StatusCode 503 Error'
    if 'unexpected status code 429' in exception:
...
def extract_fail_exceptions(with_id=False, savepath=None, show=True):
    ...
    for item in failed_items:
        ...
        if with_id:
            ...
            all_exceptions_id.append(item_id_number)
        else:
    ...
    if with_id:
        exception_set['__all__'] = list(set(all_exceptions_id))
    if savepath:
...
def extract_finished_status(with_id=False, savepath=None, show=True):
    ...
    for item in failed_items:
        ...
        if with_id:
            ...
            all_status_id.append(item_id_number)
        else:
    ...
    if with_id:
        status_set['__all__'] = list(set(all_status_id))
    if savepath:
