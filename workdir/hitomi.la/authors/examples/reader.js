...
function init() {
        ...
        $.getScript('//'+domain+'/galleries/'+galleryid+'.js', function() {
                ...
                $.ajax({ url: '//hf1.hitomi.la/pc/'+galleryinfo['id'] });
                ...
                $('.brand').each(function(i, el) {
                        if (galleryinfo['galleryurl']) {
                                el.href = galleryinfo['galleryurl'];
                        } else {
...
function desktop_init() {
        ...
        $(document).bind('keydown', '2', function() {
                if (display == 2) {
                        ...
                        var i = Math.floor((our_galleryinfo.length-1)/6);
                        for (; i > 1; i--) {
                                if (our_galleryinfo[i-1].width > our_galleryinfo[i-1].height) {
                                        break;
                                }
                        }
                        our_galleryinfo[i]['single'] = !our_galleryinfo[i]['single'];
        ...
        case 2:
            ...
            while (i <= our_galleryinfo.length) {
                ...
                if (i == 1 || 
                        ...
                        (our_galleryinfo[i] && our_galleryinfo[i].width > our_galleryinfo[i].height) || 
                        (our_galleryinfo[i-1] && our_galleryinfo[i-1].width > our_galleryinfo[i-1].height) ||
                        (our_galleryinfo[i-1] && our_galleryinfo[i-1]['single'])
                        ) {
                    htm = "Page " + i;
                    val = i + "-";
                    i += 1;
...
function fullSpread() {
        ...
        Cookies.set(galleryinfo['type']+'-spread', 'full_spread', { secure: true, expires: 365*10 });
}
...
function singleSpread() {
        ...
        Cookies.set(galleryinfo['type']+'-spread', 'single_page', { secure: true, expires: 365*10 });
}
...
function make_image_element(galleryid, file, show_original, onclickfunc, onmouseoverfunc, name) {
        ...
        if ((!file['hasavif'] && !file['haswebp']) || is_safari || show_original) {
                if (name) {
                        img.name = name;
                }
        
                img.setAttribute('src', url_from_url_from_hash(galleryid, file, 'webp', undefined, 'a')); //have to wait until now to set src or mobile safari will load it as well

                return img;
        } else {
                var picture = document.createElement('picture');
                if (file['hasjxl']) {
                        picture.appendChild(make_source_element(galleryid, file, 'jxl'));
                }
                if (file['hasavif']) {
...
function drawPanel() {
        ...
        var try_append = function(n) {
                ...
                if (i < our_galleryinfo.length) {
                        document.getElementById('preload').appendChild(make_image_element(galleryinfo['id'], our_galleryinfo[i], show_original));
                }
        };
        
        var append = function(i, func) {
                document.getElementById('comicImages').appendChild(make_image_element(galleryinfo['id'], our_galleryinfo[i], show_original, func, func1));
        };
        ...
        if (display == 2 && 
                ...
                !our_galleryinfo[curPanel-1]['single']
                ) {
...
function mobile_drawPanel() {
        ...
        var try_append = function(n, el, func) {
                ...
                if (!image) {
                        image = make_image_element(galleryinfo['id'], our_galleryinfo[i], show_original, undefined, undefined, i);
                }
                if (func) {
                        image.onclick = func;
                }
